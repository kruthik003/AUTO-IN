...
dataset = "datasets/data_analyst_interview.csv"
df = pd.read_csv(dataset)
idx = np.random.choice(10,replace = False,size = 10)
dt = df.iloc[idx]

def request_gpt(n:int,prompt: str,res:int) -> str:
    """
    ...
    sc = score(prompt,dt.iloc[n]['A'],model)
    res += sc
    return dt.iloc[n+1]['Q']
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        current_time = time()
        context += f"\nAlex: {string_words}\nK: "
        response = request_gpt(i,context,res)
        context += response
